...
function App() {
  ...
  etkili bir biçimde uygulayacaklardır.`,
    shouldDisplaySeeMoreButton: false,
    ...
    <div className={"App"}>
      ...
      <section className={"app-section"}>
        ...
        <p className={"app-section-body"} title={state.articleBody}>
          {state.isFullNoteVisible
            ? state.articleBody
            : truncateTextWithEllipsis(state.articleBody, TEXT_DISPLAY_CHARACTER_LIMIT)}
        </p>
  ...
  function onNextButtonClick(event) {
    ...
    setArticleState({
      articleTitle: "Madde 7 – Kapsamlı ve koordineli politikalar",
      articleBody,
      shouldDisplaySeeMoreButton:
        articleBody.toString().length > TEXT_DISPLAY_CHARACTER_LIMIT,
      isFullNoteVisible: !articleBody.toString().length > TEXT_DISPLAY_CHARACTER_LIMIT
    });
  }
  ...
  function onPrevButtonClick() {
    ...
    // event.stopPropogation();
    setArticleState({
      articleTitle: `Madde 5 – Devletin yükümlülükleri ve titizlikle yapması gereken inceleme ve
araştırmalar`,
